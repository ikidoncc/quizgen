...
/**
 * Generates distractors for multiple questions in batch using the OpenAI API.
 * Returns a mapping of question ID to the list of generated distractors and optional correct option overrides.
 */
export async function generateDistractorsWithOpenAI(
	questions: Omit<Question, "options" | "correctOptionId">[],
	mode: DifficultyMode,
	apiKey: string,
): Promise<Record<string, OpenAIGenerationResult>> {
	if (mode === "easy") {
		return {};
	}
...
${JSON.stringify(inputs, null, 2)}`;
	...
	const dict: Record<string, OpenAIGenerationResult> = {};
...
export async function generateQuizFromTextWithOpenAI(
	text: string,
	apiKey: string,
	quantity: number,
